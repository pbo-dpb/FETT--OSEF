...
export default defineStore("payloads", {
    ...
    actions: {
        ...
        async fetchOverview() {
            if (this.loading.includes("overview")) return;
            this.loading.push("overview");

            const response = await fetch(overviewPayloadUrl);
            const data = await response.json();

            this.overview = data;

            this.loading = this.loading.filter((item) => item !== "overview");
        },
