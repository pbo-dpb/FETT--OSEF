<template>
    <aside class="bg-white shadow-lg rounded-lg flex flex-col gap-2 w-64 px-2 py-4 shrink-0 border-2 border-solid"
        ...
        <div class="border-t-2 border-slate-100 border-solid px-4 pt-4 ">
            ...
            <div v-if="department.latest_ftes.indeterminate === 0 && department.latest_ftes.combined > 0">
                <dl class="flex flex-col gap-1">
                    <template v-for="(value, key) in { combined: department.latest_ftes.combined }" :key="key">
                        <dt class="font-medium text-xs">{{ strings[`dep_card_${key}`] }}</dt>
                        <dd class="text-lg font-light">{{ numberFormatter(Math.round(value)) }}</dd>
                    </template>
                </dl>
                <div class="text-gray-500 text-sm italic text-base/8">{{ strings.department_latest_ftes_combined_note
                    }}</div>
            </div>
            <dl class="flex flex-col gap-1" v-else>
                <template
                    v-for="(value, key) in { indeterminate: department.latest_ftes.indeterminate, term: department.latest_ftes.term, casual: department.latest_ftes.casual, student: department.latest_ftes.student }"
                    :key="key">
                    <dt class="font-medium text-xs">{{ strings[`dep_card_${key}`] }}</dt>
                    <dd class="text-lg font-light">{{ numberFormatter(Math.round(value)) }}</dd>
                </template>
