<template>
  ...
  >
    ...
    <ScrollAreaViewport class="w-full h-full rounded">
      <div class="p-4">
        ...
        <component
          ...
          :class="{
            ' cursor-pointer hover:text-sky-700 group': department.route,
            'opacity-50 cursor-not-allowed': !department.route,
          }"
        >
          <span>{{ department.name }}</span>
          <Plus
            v-if="!department.active && department.route"
            class="size-4 shrink-0 text-sky-700 opacity-0 group-hover:opacity-100"
          ></Plus>
          <CircleX
            v-if="department.active"
            class="size-4 shrink-0 text-red-700 hidden group-hover:block"
          >
          </CircleX>
          <Check
            v-if="department.active"
            class="size-4 shrink-0 text-green-700 block group-hover:hidden"
          >
          </Check>
        </component>
      </div>
    ...
    <div
      class="absolute bottom-0 z-10 w-full h-6 bg-gradient-to-b from-transparent to-white"
    />
  </ScrollAreaRoot>
</template>
<script setup>
...
const deptToListableObject = (dept) => {
  const isCurrentlySelected = currentlySelectedDepartmentIds.value.includes(
    dept.id,
  );
...
const sortedDepartments = computed(() => {
  ...
  return [...departments.value]
    .sort((a, b) =>
      a[`name_${language.value}`].localeCompare(b[`name_${language.value}`]),
    )
