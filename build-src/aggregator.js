module.exports = class Aggregator {
  ...
  preparedPaddedMonthlyDepartmentTotals = function () {
    ...
    this.datapoints
      ...
      .forEach((department_id) => {
        ...
        this.loopFunctionOverPeriod(
          ...
          (year, month) => {
            const previousMonthForDept =
              this.paddedMonthlyDepartmentTotals[department_id].length > 0
                ? this.paddedMonthlyDepartmentTotals[department_id][
                    this.paddedMonthlyDepartmentTotals[department_id].length - 1
                  ]
                : null;
            ...
            [
              ...
              let dataPointsForMonthDeptTenure = this.datapoints.filter(
                (dp) =>
                  dp.year === year &&
                  dp.month === month &&
              ...
              if (dataPointsForMonthDeptTenure.length === 0) {
                ...
                monthEntry[tenureType] = dataPointsForMonthDeptTenure
                  .map((val) => val.fte)
                  .reduce((a, b) => a + b, 0);
        ...
        this.paddedMonthlyDepartmentTotals[department_id] =
          this.paddedMonthlyDepartmentTotals[department_id].map((dp) => {
    ...
    this.paddedMonthlyTotals = this.loopFunctionOverPeriod(
      ...
      (year, month) => {
          ...
          Object.keys(priorMonthTotals[tenureType]).forEach((department_id) => {
            if (totals[tenureType][department_id] === undefined) {
              totals[tenureType][department_id] =
                priorMonthTotals[tenureType][department_id];
              ...
              if (
                totals[tenureType][department_id].source_of_dept === "data-main"
              ) {
        ...
        Object.keys(totals).forEach((tenureType) => {
          result[tenureType] = Object.values(totals[tenureType]).reduce(
            (sum, val) => sum + val.fte,
            0,
          );
        });
    ...
    while (
      ...
      (cursor.year === parseInt(settings.end_year) &&
        (period === "quarter"
          ...
          : cursor.month <=
            { 1: 3, 2: 6, 3: 9, 4: 12 }[parseInt(settings.start_quarter)]))
    ) {
      if (period === "quarter") {
  ...
  totalFtesPerFiscalYear = function (department_id = null) {
    ...
    while (
      ...
      Object.keys(monthlyTotals.find((x) => Object.keys(x).length > 3)).forEach(
        (tenureType) => {
          if (
            tenureType === "year" ||
            tenureType === "month" ||
      ...
      Object.keys(monthlyTotals.find((x) => !x.unreported)).forEach(
        (tenureType) => {
          ...
          const monthlyTotalsForTenureArray = monthlyTotals
            .filter(
              (mt) => mt.year === year && quarterMonths.includes(mt.month),
            )
            .map((val) => val[tenureType] || 0);
          if (monthlyTotalsForTenureArray.length) {
            // Average over all months in the quarter
            totals[tenureType] =
              monthlyTotalsForTenureArray.reduce((a, b) => a + b, 0) /
  ...
  totalFtesPerQuarter = function () {
    ...
    this._totalFtesPerQuarter = this.transformMonthlyTotalsToQuarterlyTotals(
      this.settings,
      monthlyTotals,
    );
    return this._totalFtesPerQuarter;
  ...
  latestTotalFtesForDepartment = function (department_id) {
    const reports = this.paddedMonthlyDepartmentTotals[department_id].filter(
      (monthEntry) => monthEntry.unreported === false,
    );
